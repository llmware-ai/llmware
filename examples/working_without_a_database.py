...
def analyze_contracts_on_the_fly():
    ...
    prompter = Prompt(save_state=True).load_model("claude-instant-v1", api_key=anthropic_api_key)
...
def create_datasets_from_prompt_history():
    ...
    # Start a new PromptState session
    #PromptState().initiate_new_state_session()

    # Create a Prompt
    prompter = Prompt(llm_name="gpt-4", llm_api_key=openai_api_key, save_state=True)
...
def try_all_prompt_instructions():
    ...
    # Create a prompt
    prompter = Prompt(save_state=True).load_model("gpt-3.5-turbo", api_key=openai_api_key)
...
def use_specific_prompt_instructions():
...
def create_custom_prompt():
    ...
    # Create a new prompt
    prompter = Prompt(save_state=True,prompt_catalog=prompt_catalog).load_model("gpt-3.5-turbo", api_key=openai_api_key)

    # Prompt the LLM
    response = prompter.prompt_from_catalog("Where was Joe Biden born?",context=test_sample_short, prompt_name="my_prompt")
    print("\nLLM Response:\n" + response["llm_response"])

analyze_contracts_on_the_fly()
create_datasets_from_prompt_history()
parsing_with_no_library()
parse_all_to_json()
try_all_prompt_instructions()
use_specific_prompt_instructions()
