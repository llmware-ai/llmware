...
# Load an llmware model from Hugging Face and generate vector embeddings which you can store in whatever vector DB you like
# See the embeddings.py and other examples for how to use llmware's supported vector DBs and perform semantic queries
def use_llmware_hf_models_for_embedding():

    # llmware industry models currently published on HuggingFace (more will be coming!)
    llmware_industry_models = ["llmware/industry-bert-sec-v0.1",
...
def override_generative_model_weights_with_custom_fine_tuned_model():
    ...
    print (f"\n > Overring model '{model_name}' to use custom-trained weights from '{custom_model_name}'...")
...
def use_sentence_transformers_model_for_embedding(library_name, model_name):
    ...
    query_term = "salary"
    print (f"\n > Performing query for {query_term}...")
    query= Query(library=library, embedding_model_name=model_name, embedding_model=sbert_model,
