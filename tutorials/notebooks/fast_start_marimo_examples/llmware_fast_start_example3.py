...
@app.cell
def __(mo):
    mo.md("""     Fast Start Example #3 - Prompts & Model Catalog - how to build prompts and run inferences
...
def __():
    ...
    return ModelCatalog, Prompt, time


@app.cell
def __(mo):
    mo.md(""" This is a set of useful test questions to do a 'hello world' but there is nothing special about the
...
@app.cell
def __():
    test_list = [{"query": "What is the total amount of the invoice?",
    "answer": "$22,500.00",
    ...
    "THANK YOU FOR YOUR BUSINESS!  INVOICE INVOICE # 0001 DATE 01/01/2022 FOR Alpha Project P.O. # 1000"},{"query": "What was the amount of the trade surplus?",
...
@app.cell
def __(mo):
    mo.md("""Step 1 - we will pick a model from the ModelCatalog """)
    return
...
def __(ModelCatalog):
    ...
    for j, models in enumerate(llm_local_models):
        print("models: ", j, models["model_name"], models["model_family"])
...
@app.cell
def __(mo):
    mo.md( """ This is the main example script - it loads the question list, loads the model and executes the prompts. """)
    return


@app.cell
def __(Prompt, model_name, test_list, time):
    t0 = time.time()
    ...
    return entries, i, llm_response, output, prompter, t0, t1, t2
